...
const gstrFG = (res, path, iduser) => {
  if (1) // TODO: make login work...
    http.request({ host: 'farm.akfn.net', path }, (res2) => {
      let str = '';
      res2.on('data', (chunk) => str += chunk);
      res2.on('end', () => {
        const obj = JSON.parse(str.replace(/([a-zA-Z]+)/ig, '"$1"'));
        obj.location = 1;
        // TODO: change to POST with body
        if (path !== '/status') {
          const ddh = path.split('-')
          let idequip = 0, status = '';
          if (ddh[0].endsWith('pump'))
            idequip = 2;
          else if (ddh[0].endsWith('light'))
            idequip = 1;
          if (ddh[1] === 'on')
            status = 'off';
          else if (ddh[1] === 'off')
...
let lastUpdate = 0, statusCache = null;

router.get('/', (req, res) => {
  const { auth_token, auth_expiry } = req.session;
  if (auth_expiry < new Date().getTime())
    res.redirect('/account/signin');
  else
    res.render('index', { auth_token });
});
...
router.post('/log', (req, res) => {
  ...
  db.pool.query('select b.name as `name`, a.`status` as `status`, a.created as created, c.nickname as `by` from usagelog a join equipment b on a.idequipment=b.idequipment join user c on a.createdBy=c.iduser where a.idlocation=? order by a.created desc limit 4',
    [location], (err, dres) => {
      if (err) {
        console.log(err);
        res.setStatus(500);
